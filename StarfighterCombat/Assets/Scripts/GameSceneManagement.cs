...
public class GameSceneManagement : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (DataManager.Instance != null && DataManager.Instance.selectedCraftPrefab != null)
        {
            ...
            //add the CraftController component to the instantiated craft.
            craft.AddComponent<CraftController>();

            CraftController craftController = craft.AddComponent<CraftController>();

            if (moveLeftButton && moveRightButton)
            {
                craftController.SetMovementButtons(moveLeftButton, moveRightButton);
            }
